...
## 2026-07-05 Codex result update

The 2026-07-05 Codex results were rechecked after being added back into this project.

- 58 unique files were moved into the main classified analysis directories.
- 36 duplicate files remain only in `99_codex_results_from_c` because equivalent same-name/same-size copies already exist in the classified directories.
- Newly promoted files supplement early inferCNV subclone generation, early B/T immune subtype mapping, broad integrated_oc ligand-receptor tables, object-inspection scripts, external-data preparation scripts, and package/OmniPath logs.
- Use `D:\OC_spatiogenomics\infercnv\file_reclassification_manifest_2026-07-10_with_7-5.csv` as the current whole-project file index.
- Use `D:\OC_spatiogenomics\infercnv\99_codex_results_from_c\2026-07-05_reclassification_manifest.csv` to check exactly how the 2026-07-05 files were handled.

## Locked source folders still present

Two old root-level source folders were copied into classified destinations and verified by file count and total bytes, but Windows did not allow deleting the old root-level copies because files were in use or access was denied:
...
Also check whether the 2026-07-05 early inferCNV/integrated_oc outputs are consistent with the later refined analyses, rather than treating them as independent final evidence. Prioritize the reports and core evidence files listed above; use the 2026-07-05 materials as provenance, early subclone/LR context, and supplemental checks.
